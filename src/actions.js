...
function parseAction(text, base) {
  ...
  // (4b) 漢字・カタカナで取れなかったときだけ、ひらがな表記の名前を許す（「たなかさんが」）。
  //      文字起こしは聞き慣れない姓や下の名前をひらがなで出すことがある。
  //      行頭・句読点・空白の直後だけから取る。助詞と名前の境目はひらがな同士では
  //      決められない（「今週中にたなか」→「にたなか」）ので、曖昧な位置では取らない。
  if (!assignee && (m = body.match(/(?:^|[、。，,\s「」（）()])([ぁ-ん]{2,5}(?:さん|氏|様|部長|課長|主任))\s*(?:が|は|に|へ)/))) {
    assignee = m[1];
  }
